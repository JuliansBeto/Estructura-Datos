...
void palindromos::generaexacto(char frasepura[], int medida, string palabracompleta) //genera un arreglo de medida igual a la del arreglo original menos los espacios, esto para tener la palabra de corrido y hacer una comparacion mas sencilla
{
    int j = 0;
    int x = 0;
...
bool palindromos::decision(char frasepura[], int ini, int fin) //decidir si es palindromo
{
    ...
    {
        palindromos::decision(frasepura, ini+1, fin-1);//si la posición actual es confirmada, confirmar las siguientes, se vuelve a invocar con valores cada vez mas cercanos al centro de la palabra
    }
    else
    {
